...
{{< include /_data/projects.qmd >}}

{{< include /_data/orgs.qmd >}}

:::{.about-entity .column-page}

![](/brand/logo/logo-with-text.png){alt="The hubverse" width="75%" style="display: block; margin-left: auto; margin-right: auto;"}
...
[**hubverse software**](tools/software.qmd) is written in R, Python, and JavaScript,
and provides tools for all phases in the life cycle of a hub.
...
:::{ style="font-size: 1.4rem;"}
 ...
 - {{< fa chart-line >}} **visualize** forecasts with [hubVis](https://hubverse-org.github.io/hubVis)
 - ... and more!
...
:::{#testimonial-carousel .make-carousel}
{{< partial /_partials/testimonial.qmd hmsc >}}
{{< partial /_partials/testimonial.qmd sjf >}}
{{< partial /_partials/testimonial.qmd acefa >}}
:::


## Built with the hubverse {#showcase .column-screen-inset}

Cool projects made possible by the hubverse, built by teams across the community.

:::{#showcase-carousel .make-carousel}
{{< partial /_partials/showcase.qmd projects >}}
:::

```{=html}
<script>
// Turn each .make-carousel container into an auto-rotating Bootstrap carousel,
// using its .card children as slides. Falls back to stacked cards if JS/Bootstrap
// is unavailable.
(function () {
  function carouselify(container, interval) {
    var cards = Array.prototype.slice.call(container.querySelectorAll('.card'));
    if (cards.length < 2) return;
    var id = container.id || ('hub-carousel-' + Math.random().toString(36).slice(2));
    container.id = id;
    container.classList.add('carousel', 'slide', 'hub-carousel');
    ...
    cards.forEach(function (_, i) {
      ...
      dot.type = 'button';
      dot.setAttribute('data-bs-target', '#' + id);
    ...
    function control(dir, label) {
      var btn = document.createElement('button');
      btn.className = 'carousel-control-' + dir;
      btn.type = 'button';
      btn.setAttribute('data-bs-target', '#' + id);
      btn.setAttribute('data-bs-slide', dir);
      btn.innerHTML = '<span class="carousel-control-' + dir + '-icon" aria-hidden="true"></span>' +
    ...
    container.appendChild(control('prev', 'Previous'));
    container.appendChild(control('next', 'Next'));

    if (window.bootstrap && window.bootstrap.Carousel) {
      new window.bootstrap.Carousel(container, { interval: interval, ride: 'carousel', pause: 'hover' });
    }
  }

  // Give every slide's card the same height (the tallest) so the carousel does
  // not jump as it rotates. Hidden slides are measured by momentarily showing
  // them; re-run after images load and on resize.
  function equalize(container) {
    var items = Array.prototype.slice.call(container.querySelectorAll('.carousel-item'));
    items.forEach(function (it) { var c = it.querySelector('.card'); if (c) c.style.minHeight = ''; });
    var max = 0;
    items.forEach(function (it) {
      var prev = it.style.display;
      it.style.display = 'block';
      var c = it.querySelector('.card');
      if (c) max = Math.max(max, c.offsetHeight);
      it.style.display = prev;
    });
    items.forEach(function (it) { var c = it.querySelector('.card'); if (c) c.style.minHeight = max + 'px'; });
  }

  var carousels = [];
  function equalizeAll() { carousels.forEach(equalize); }

  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.make-carousel').forEach(function (c) {
      carouselify(c, 7000);
      carousels.push(c);
    });
    equalizeAll();
  ...
  window.addEventListener('resize', function () {
    ...
    resizeTimer = setTimeout(equalizeAll, 200);
  });
})();
</script>
```
...
{{< partial /_partials/orgs.qmd orgs >}}
...
```{=html}
<script>
// Shuffle the organization cards on each page load so no organization is
// consistently listed first (Fisher-Yates).
document.addEventListener('DOMContentLoaded', function () {
  var grid = document.getElementById('orgs');
  if (!grid) return;
  var cards = Array.prototype.slice.call(grid.querySelectorAll('.card.org'));
